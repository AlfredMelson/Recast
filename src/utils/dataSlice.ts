...
import SampleJson from '../data/sample/sample.json'
import { Path, EditMode, EditType } from '../editor/types'
import { isArray } from '../editor/objects/types'
  ...
  reducers: {
    ...
    deletePath: (state: State, action: PayloadAction<Path>) => {
      ...
      if (parentPath.length === 0) {
        ...
        if (isArray(data)) {
          // Array
          state.data = data.filter((_: any, index: any) => `${index}` !== name)
        } else {
          // Object
    ...
    updateDataOfPath: (state: State, action: PayloadAction<{ data: any; path: Path }>) => {
      const { data, path } = action.payload
      _.set(state.data, path, data)
    },
    updateKeyOfPath: (state: State, action: PayloadAction<{ key: string; path: Path }>) => {
      const { key: newKey, path } = action.payload
      // get
      const [parentPath, name] = getParentPathAndName(path)
      const targetData = parentPath.length === 0 ? state.data : _.get(state.data, parentPath)
      const newObject = {}
    ...
    insertDataAfterPath: (state: State, action: PayloadAction<Path>) => {
      ...
      // get
      const newData = parentPath.length === 0 ? state.data : _.get(state.data, parentPath)
      // insert
      if (isArray(newData)) {
