 ...
 */
export function showSortModal(container, json) {
  const paths = Array.isArray(json) ? getChildPaths(json) : ['']
  const selectedPath =
    options && options.path && contains(paths, options.path) ? options.path : paths[0]
  ...
  picoModal({
    ...
    .afterCreate(modal => {
      ...
      function setDirection(value) {
        ...
        direction.className =
          'jsoneditor-button-group jsoneditor-button-group-value-' + direction.value
      }
