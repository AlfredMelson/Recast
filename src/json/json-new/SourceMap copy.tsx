export function SourceMap(source: string | any[], _: any, options: { bigint: any }) {
  const escapedChars = {
    b: '\b',
    f: '\f',
    n: '\n',
  ...
  return {
    data: parseSourse('', true),
    pointers: pointers,
  }

  function parseSourse(ptr: string, topLevel?: boolean) {
    whitespace()
    let data: string | number | bigint | boolean | any[] | Record<string, unknown>
    map(ptr, 'value')
    const char = getChar()
    switch (char) {
      case 't':
  ...
  function parseArray(ptr: string) {
    whitespace()
    const arr = []
    let i = 0
  ...
  function parseObject(ptr: string) {
    ...
    while (true) {
      ...
      mapLoc(propPtr, 'key', location)
      map(propPtr, 'keyEnd')
      whitespace()
      if (getChar() != ':') wasUnexpectedToken()
      whitespace()
      obj[key] = parseSourse(propPtr)
      whitespace()
      const char = getChar()
      if (char == '}') break
      if (char != ',') wasUnexpectedToken()
  ...
  function mapLoc(
    ptr: string,
    prop: string,
    location: { line: number; column: number; pos: number }
  ) {
    pointers[ptr] = pointers[ptr] || {}
    pointers[ptr][prop] = location
  }
...
export function ExportStringify(data, options) {
  ...
  switch (typeof whitespace) {
    case 'number':
      const len = whitespace > 10 ? 10 : whitespace < 0 ? 0 : Math.floor(whitespace)
  ...
  function _stringify(_data, level, ptr) {
    map(ptr, 'value')
    switch (typeof _data) {
      case 'number':
      case 'bigint':
    ...
    function stringifyMapSet(isSet?: boolean) {
      if (_data.size) {
        out('{')
        const propLvl = level + 1
        let first = true
