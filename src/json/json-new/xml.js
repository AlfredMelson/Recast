  ...
  ;(t.prototype.xml = function (t, o) {
    ...
    for (s = 0; s < a; s++)
      i[s].search(/<!/) > -1
        ? ((l += u[n] + i[s]),
          (r = !0),
          (i[s].search(/-->/) > -1 || i[s].search(/\]>/) > -1 || i[s].search(/!DOCTYPE/) > -1) &&
            (r = !1))
        : i[s].search(/-->/) > -1 || i[s].search(/\]>/) > -1
        ? ((l += i[s]), (r = !1))
        : /^<\w/.exec(i[s - 1]) &&
          /^<\/\w/.exec(i[s]) &&
          /^<[\w:\-\.\,]+/.exec(i[s - 1]) == /^<\/[\w:\-\.\,]+/.exec(i[s])[0].replace('/', '')
        ? ((l += i[s]), r || n--)
        : i[s].search(/<\w/) > -1 && -1 == i[s].search(/<\//) && -1 == i[s].search(/\/>/)
        ? (l = l += r ? i[s] : u[n++] + i[s])
        ...
        : i[s].search(/\/>/) > -1
        ? (l = l += r ? i[s] : u[n] + i[s])
        : i[s].search(/<\?/) > -1 || i[s].search(/xmlns\:/) > -1 || i[s].search(/xmlns\=/) > -1
        ? (l += u[n] + i[s])
        : (l += i[s])
    return '\n' == l[0] ? l.slice(1) : l
  }),
    ...
    (t.prototype.sql = function (t, a) {
      ...
      for (n = l.length, p = 0; p < n; p++) {
        ;(c = o(l[p], c)),
          /\s{0,}\s{0,}SELECT\s{0,}/.exec(l[p]) && (l[p] = l[p].replace(/\,/g, ',\n' + u + u)),
          /\s{0,}\s{0,}SET\s{0,}/.exec(l[p]) && (l[p] = l[p].replace(/\,/g, ',\n' + u + u)),
...
function preInitEditors() {
  if ('json' == toolstype)
    createJsonEditor(), cleanJSONEditor('all'), setDataInInputEditor(getFromLocalStorage())
    ...
    $('#default_file').change(function () {
      ;(formdata = new FormData()),
        $(this).prop('files').length > 0 &&
          ((file = $(this).prop('files')[0]), formdata.append('userfile', file), readSelectedFile())
    })
}
...
function updateFullScreenIcons(e) {
  isFullScreen
    ? 'input' == e
      ? ($('#inputFullScreen').hide(), $('#inputCloseScreen').show(), (fullscreenEditor = 'input'))
      : 'output' == e &&
        ($('#outputFullScreen').hide(),
        $('#outputCloseScreen').show(),
        (fullscreenEditor = 'output'))
    ...
    : 'output' == e && ($('#outputFullScreen').show(), $('#outputCloseScreen').hide())
}
function getCookie(e) {
  for (var t = e + '=', o = document.cookie.split(';'), i = 0; i < o.length; i++) {
    for (var a = o[i]; ' ' == a.charAt(0); ) a = a.substring(1)
              ...
              "Data saved successfully - <a class='white' href ='" +
                (o =
                  'index' == $('#viewname').val().trim()
                    ...
                    : 'https://' + location.host + '/' + $('#viewname').val().trim() + '/' + t) +
                "'>" +
                o +
                '</a>',
              !1
...
$(function () {
  if (($(window).scrollTop(0), 'recentLinks' == (toolstype = $('#type').val().trim()))) {
...
function saveToLocalStorage(e) {
  localStorage &&
    (localStorage.setItem($('#viewname').val(), ''), localStorage.setItem($('#viewname').val(), e))
}
  ...
  if (0 != e.trim().length) {
    ...
    for (var r = 1; r < o.length; r++) {
      for (rows += '<tr>', a = 0; a < i[0].length; a++)
        a < o[r].length ? (rows += '<td>' + o[r][a] + '</td>') : (rows += '<td>&nbsp</td>')
      rows += '</tr>'
    }
    return (
      htmlOutput(
  ...
  if ('function' == typeof JSONEditor) {
    if (0 != $('#outputeditor1').length) {
      var e = document.getElementById('outputeditor1'),
        t = (outputEditorForTree = new JSONEditor(
          e,
          {
            mode: 'view',
            onError: function (e) {
          ...
          null
        )).menu.getElementsByClassName('poweredBy')
      $(t).hide(), xmlTreeView()
    }
  } else $.loadScript('dist/js/vendor/jsoneditor-xmltree.js', createLazyJSONEditor)
}
...
function addXMLInputIconstoEditor() {
  var e = $('.rightmenu')
  $(e).replaceWith(
    '<div class="btn-group btn-group-sm right"><div id="fileopen" class="cursor-pointer btn-sm fa fa-folder-open" title="Open File" ></div><div class="cursor-pointer btn-sm fa fa-floppy-o" title="Save online" onclick="Action_Save();"></div><div class="cursor-pointer btn-sm fa fa-check" title="XML Validator" onclick="validateXML();"></div><div class="cursor-pointer btn-sm fa fa-print" title="Print XML" onclick="printXML();"></div><div class="cursor-pointer btn-sm fa fa-times" title="Clear" onclick="clearXMLEditor(\'inputxmleditor\')"></div><div id="inputcopy" title="Copy to Clipboard" class="cursor-pointer btn-sm btn-shrink fa fa-files-o"></div><div id="inputFullScreen" title="fullscreen" onclick="addOverlay(\'input\');" class="cursor-pointer btn-sm btn-fullscreen fa fa-arrows-alt"></div><div id="inputCloseScreen" title="Close" onclick="removeOverlay(\'input\');" style="display:none" class="cursor-pointer btn-sm btn-fullscreen fa fa-window-close"></div></div>'
  ),
    $(e).show(),
...
function addXMLOutputIconstoEditor(e) {
  ...
  $(t).after(
    '<div id="outputToolBar" class="btn-group btn-group-sm right"><div class="cursor-pointer tree-rotate-180 btn-sm fa fa-tree" title="Tree view" onclick="xmlTreeView(),showEditor();"></div><div class="cursor-pointer btn-sm fa fa-times" title="Clear" onclick="clearXMLEditor()"></div><div class="cursor-pointer btn-sm fa fa-download" title="Download" onclick="downloadinXMLTool();"></div><div id="outputcopy" title="Copy to Clipboard" class="cursor-pointer btn-sm btn-shrink fa fa-files-o"></div><div id="outputFullScreen" title="fullscreen" onclick="addOverlay(\'output\');" class="cursor-pointer btn-sm btn-fullscreen fa fa-arrows-alt"></div><div id="outputCloseScreen" title="Close" onclick="removeOverlay(\'output\');" style="display:none" class="cursor-pointer btn-sm btn-fullscreen fa fa-window-close"></div></div>'
  ),
    e ? $(t).show() : $(t).hide(),
...
function loadXMLSampleData() {
  0 != $('#inputeditor').length && inputEditor.setValue(vkbeautify.xml(getXMLSampleData())),
    defaultAction()
}
...
function xmlBeautify() {
  inputEditor.getSession().setMode('ace/mode/xml'),
...
function xmlTreeView() {
  ...
  hideMessage()
  try {
    if (null == outputEditorForTree || !outputEditorForTree instanceof JSONEditor)
      return void createLazyJSONEditor()
    var t = new X2JS()
    outputEditorForTree.setText(JSON.stringify(t.xml_str2json(e))),
      $('#outputheading').text('XML Tree'),
      $('.jsoneditor-search').css('right', '10px')
  } catch (e) {
    console.log(e)
  }
...
function xmlMinify() {
  ...
  try {
    outputEditor.getSession().setUseWrapMode(!0),
      outputEditor.setValue(vkbeautify.xmlmin(e)),
      $('#outputheading').text('XML Minify')
  } catch (e) {
    validateXML()
  }
  last_action = 'beautify'
...
function xmltojson(e = 'XML to JSON') {
  ;(isXmlData = !1),
    ...
    'RSS to JSON' === e || 'XML to JSON' === e ? (xmlConvertTitle = e) : (e = xmlConvertTitle)
  var t = inputEditor.getValue()
  if (t.trim().length > 0)
    try {
      if ('function' != typeof X2JS)
        return void $.loadScript('dist/js/vendor/xml2json.min.js', xmltojson)
      var o = new X2JS()
      outputEditor.setValue(vkbeautify.json(JSON.stringify(o.xml_str2json(t)))),
        $('#outputheading').text(e),
        (last_action = 'xmltojson')
    } catch (e) {
      setMessage('error', e)
    }
}
...
function xmlTotsv() {
  ...
  if (e.trim().length > 0)
    try {
      var t = new X2JS().xml_str2json(e),
        o = jsonToCsv(t, ',', !0, !1, !1)
      outputEditor.setValue(getCSVTOTSV(o)),
        $('#outputheading').text('XML to TSV'),
        (last_action = 'xmltocsv')
    } catch (e) {
      setMessage('(XML is not valid) : error', e)
    }
}
...
function xmlToyaml(e) {
  'function' != typeof X2JS && $.loadScript('dist/js/vendor/xml2json.min.js'),
    'object' != typeof YAML
      ? $.loadScript('dist/js/vendor/yaml.min.js', function () {
          processXMLToYaml(e)
        })
      : processXMLToYaml(e)
}
function plainJSONinXMLtoJSON() {
  if (validateXML()) {
    ...
    var e = outputEditor.getValue(),
      t = $.parseJSON(e)
    localStorage.setItem('jsondata', JSON.stringify(t, null, 2)),
      window.open('/jsonview', '_blank').focus()
  }
}
function downloadinXMLTool() {
  if ('function' == typeof saveAs) {
    ...
    if (0 != e.trim().length) {
      ...
      ;(filename = 'data.xml'),
        'xmltojson' == last_action && (filename = 'json.txt'),
        saveAs(t, filename)
    } else setMessage('error', 'Sorry Result is Empty')
...
function updateXMLOutput(e) {
  null != inputEditor &&
    0 != inputEditor.getValue().trim().length &&
    ('beautify' == e
      ? xmlBeautify()
      : 'minify' == e
      ? minifyJSON()
...
function plainXML() {
  xmlBeautify(),
    localStorage.setItem('xmldata', outputEditor.getValue()),
    window.open('/xmlview', '_blank').focus()
}
function showEditor(e = 'tree') {
  'ace' == e
    ? ($('#outputeditorDiv').removeClass('hide'), $('#outputeditor1Div').addClass('hide'))
    : ($('#outputeditor1Div').removeClass('hide'), $('#outputeditorDiv').addClass('hide'))
}
  ...
  if (e.trim().length > 0)
    try {
      ...
      outputEditor.setValue(o),
        (last_action = 'xmlToJava'),
        $('#outputheading').text('XML to JAVA'),
        (last_action = 'xmltojson')
    } catch (e) {
      setMessage('error', e)
    }
...
function plainXMLtoJava() {
  xmlToJava(),
    ...
    window.open('/javaview', '_blank').focus()
}
