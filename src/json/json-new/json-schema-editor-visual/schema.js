// https://github.dev/Open-Federation/json-schema-editor-visual
...
export default {
  ...
  changeNameAction: function (state, action, oldState) {
    ...
    let openKeys = [].concat(keys, value, 'properties').join(utils.JSONPATHJOINCHAR)
    let oldOpenKeys = [].concat(keys, name, 'properties').join(utils.JSONPATHJOINCHAR)
  ...
  setOpenValueAction: function (state, action, oldState) {
    const keys = action.key.join(utils.JSONPATHJOINCHAR)
