...
const base64 = schema =>
  schema.type === 'string' && schema.media && schema.media.binaryEncoding === 'base64' && 'base64'
...
const date = schema =>
  ['string', 'integer'].includes(schema.type) &&
  ['date', 'time', 'datetime-local'].includes(schema.format) &&
  'datetime'

/* Use a specialized editor for starratings */
const starratings = schema =>
  ['string', 'integer'].includes(schema.type) &&
  ['starrating', 'rating'].includes(schema.format) &&
  'starrating'
...
const autoComplete = schema =>
  schema.type === 'string' && schema.format === 'autocomplete' && 'autocomplete'
...
const xhtml = schema =>
  schema.type === 'string' && ['xhtml', 'bbcode'].includes(schema.format) && 'sceditor'
...
const ip = schema =>
  schema.type === 'string' && ['ip', 'ipv4', 'ipv6', 'hostname'].includes(schema.format) && 'ip'
