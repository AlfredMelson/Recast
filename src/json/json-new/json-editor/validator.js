...
export class Validator {
    ...
    this._validateSubSchema = {
      ...
      type(schema, value, path) {
        ...
        if (Array.isArray(schema.type)) {
          ...
          /* Simple type */
          if (
            ['date', 'time', 'datetime-local'].includes(schema.format) &&
            schema.type === 'integer'
          ) {
          ...
          return [
            {
              ...
              property: 'pattern',
              message:
                schema.options && schema.options.patternmessage
      ...
      additionalProperties(schema, value, path, validatedProperties) {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (!schema.additionalProperties) {
            ...
            errors.push(
              ...this._validateSchema(schema.additionalProperties, value[k], `${path}.${k}`)
            )
          }
        }
        return errors
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      if (schema.anyOf) {
        ...
        for (const subSchema of schema.anyOf) {
          ...
          if (
            subFit.match > bestFit.match ||
            (subFit.match === bestFit.match && subFit.extra < bestFit.extra)
          ) {
            bestFit = subFit
          }
        }
  ...
  _getSchema(schema) {
    return typeof schema === 'undefined'
      ? extend({}, this.jsoneditor.expandRefs(this.schema))
      : schema
  }
    ...
    if (typeof value === 'number') {
      ...
      Object.keys(schema).forEach(key => {
        if (this._validateObjectSubSchema[key]) {
          errors.push(
            ...this._validateObjectSubSchema[key].call(
              this,
              schema,
              value,
              path,
      ...
      Object.keys(schema).forEach(key => {
        if (typeof this._validateObjectSubSchema2[key] !== 'undefined') {
          errors.push(
            ...this._validateObjectSubSchema2[key].call(
              this,
              schema,
              value,
              path,
  ...
  _validateDateTimeSubSchema(schema, value, path) {
    ...
    const dateFormat =
      editor && editor.flatpickr ? editor.flatpickr.config.dateFormat : format[schema.format]
