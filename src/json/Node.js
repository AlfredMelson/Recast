...
export class Node {
  constructor(editor, params) {
    ...
    this._debouncedOnChangeValue = debounce(
      this._onChangeValue.bind(this),
      Node.prototype.DEBOUNCE_INTERVAL
    )
    this._debouncedOnChangeField = debounce(
  ...
  _updateEditability() {
    ...
    if (this.editor) {
      ...
      if (
        (this.editor.options.mode === 'tree' || this.editor.options.mode === 'form') &&
  ...
  updateError() {
    ...
    if (error && this.dom && this.dom.tr) {
      ...
      const createPopup = destroyOnMouseOut => {
        ...
        this.dom.popupAnchor = createAbsoluteAnchor(
          button,
  ...
  _onEvent(event) {
    ...
    const isValue =
      element === this.dom.value || element === this.dom.checkbox || element === this.dom.select
  ...
  onKeyDown(event) {
    ...
    let multiselection
    const selectedNodes =
      this.editor.multiselection.nodes.length > 0 ? this.editor.multiselection.nodes : [this]
        ...
        submenu: [
          {
            text: translate('auto'),
            className:
              'jsoneditor-type-auto' + (this.type === 'auto' ? ' jsoneditor-selected' : ''),
          ...
          },
          {
            ...
            className:
              'jsoneditor-type-array' + (this.type === 'array' ? ' jsoneditor-selected' : ''),
          ...
          },
          {
            ...
            className:
              'jsoneditor-type-object' + (this.type === 'object' ? ' jsoneditor-selected' : ''),
          ...
          },
          {
            ...
            className:
              'jsoneditor-type-string' + (this.type === 'string' ? ' jsoneditor-selected' : ''),
  ...
  updateNodeName() {
    ...
    if (this.type === 'object' || this.type === 'array') {
      ...
      this.dom.value.textContent =
        this.type === 'object' ? '{' + (nodeName || count) + '}' : '[' + (nodeName || count) + ']'
    }
  }
  ...
  if (mouseY < topThis) {
    ...
    if (trFirst) {
      ...
      do {
        ...
        if (trNext) {
          bottomNext = trNext.nextSibling ? getAbsoluteTop(trNext.nextSibling) : 0
      ...
      if (nodeNext && nodeNext.parent) {
        ...
        while (levelNext < level && trPrev) {
          ...
          const isDraggedNode = nodes.some(
            node => node === nodePrev || nodePrev.isDescendantOf(node)
          )
