  ...
  const createDropDownController = (elem, rs) => {
    ...
    const p = {
      ...
      refresh: function (token, array) {
        ...
        rows = []
        const filterFn =
          typeof config.filter === 'function' ? config.filter : defaultFilterFunction[config.filter]
        ...
        if (
          rows.length === 1 &&
          ((token.toLowerCase() === rows[0].__hint.toLowerCase() && !config.caseSensitive) ||
  ...
  const rs = {
    ...
    repaint: function (element) {
      ...
      for (let i = 0; i < optionsLength; i++) {
        ...
        if (
          (!config.caseSensitive && opt.toLowerCase().indexOf(token.toLowerCase()) === 0) ||
      ...
      if (!wasDropDownHidden) {
        this.elementHint.style.width =
          calculateWidthForText(this.elementHint.innerText) + dropDown.clientWidth + 'px'
      }
    },
  }
